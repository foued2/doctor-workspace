from typing import List
import math


class Solution:
    @staticmethod
    def areaOfMaxDiagonal(dimensions: List[List[int]]) -> int:
        # Initialize variables to track the largest diagonal and corresponding area
        largest_diagonal = 0
        maximum_area = 0

        # Iterate through each rectangle's dimensions
        for rectangle in dimensions:
            # Calculate the diagonal length using the Pythagorean theorem
            diagonal = math.sqrt(rectangle[0] ** 2 + rectangle[1] ** 2)
            # Calculate the area of the rectangle
            area = rectangle[0] * rectangle[1]

            # Check if the current diagonal is greater than the largest diagonal found so far
            if diagonal > largest_diagonal:
                # Update the maximum area and largest diagonal
                maximum_area = area
                largest_diagonal = diagonal
            # Check if the current diagonal is equal to the largest diagonal found so far
            elif diagonal == largest_diagonal:
                # Update the maximum area if the current rectangle's area is greater
                maximum_area = max(maximum_area, area)

        # Return the maximum area of a rectangle with the longest diagonal among all rectangles provided
        return maximum_area


# Example usage:
dimensions = [[4, 7], [8, 9], [5, 3], [6, 10], [2, 9], [3, 10], [2, 2], [5, 8], [5, 10], [5, 6], [8, 9], [10, 7],
              [8, 9], [3, 7], [2, 6], [5, 1], [7, 4], [1, 10], [1, 7], [6, 9], [3, 3], [4, 6], [8, 2], [10, 6], [7, 9],
              [9, 2], [1, 2], [3, 8], [10, 2], [4, 1], [9, 7], [10, 3], [6, 9], [9, 8], [7, 7], [5, 7], [5, 4], [6, 5],
              [1, 8], [2, 3], [7, 10], [3, 9], [5, 7], [2, 4], [5, 6], [9, 5], [8, 8], [8, 10], [6, 8], [5, 1], [10, 8],
              [7, 4], [2, 1], [2, 7], [10, 3], [2, 5], [7, 6], [10, 5], [10, 9], [5, 7], [10, 6], [4, 3], [10, 4],
              [1, 5], [8, 9], [3, 1], [2, 5], [9, 10], [6, 6], [5, 10], [10, 2], [6, 10], [1, 1], [8, 6], [1, 7],
              [6, 3], [9, 3], [1, 4], [1, 1], [10, 4], [7, 9], [4, 5], [2, 8], [7, 9], [7, 3], [4, 9], [2, 8], [4, 6],
              [9, 1], [8, 4], [2, 4], [7, 8], [3, 5], [7, 6], [8, 6], [4, 7], [25, 60], [39, 52], [16, 63], [33, 56]]
print(Solution.areaOfMaxDiagonal(dimensions))  # Expected output: 2028


class Solution:
    @staticmethod
    def areaOfMaxDiagonal(dimensions: List[List[int]]) -> int:
        a, b = max(dimensions, key=lambda x: (x[0] * x[0] + x[1] * x[1], (x[0] * x[1])))
        return a * b


print(Solution.areaOfMaxDiagonal([[2, 6], [5, 1], [3, 10], [8, 4]]))
